...
With all of this work, the spectrometer (with the two gratings and two cameras) was returned to service.  Reverse engineering often requires conservative planning often with significant engineering on its own.  With a slow and careful approach it is possible to fix most things, however it is truly a question of time and effort. With this, I definitely have an appreciation for both engineering design and the process by which firms try to reverse engineering it.

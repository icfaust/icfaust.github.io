...
If there is anything I've learned while doing circuit testing is that both capacitive and inductive coupling of circuits are often the greatest enemies.  Capacitive coupling is often the greatest enemy of high-frequency circuits and can be mitigated with proper shielding and distance.  The other, inductive coupling, is much less often discussed.  Inductive coupling is often seen known by another name in specific circumstances - a 'ground loop'.  Ground loops occur when two grounding points are at different potentials, leading to undesired noise between the two points. This has a surprisingly awesome write-up of <a href="https://en.wikipedia.org/wiki/Ground_loop_(electricity)"> 'ground loops' </a> on wikipedia that I recommend.

...
The standard optics for the <a href="https://www.raspberrypi.org/products/camera-module-v2/"> 'Raspberry Pi camera'</a> can be removed with a set of pliers, which exposes the underneath CCD. The screw holes on either side of the CCD mount the M12 optics, the optics themselves are sized to fit a M12 hole with .5mm pitch (which is really fine, and requires a special tap for 3D mounting).  The M12 optics are typical of CCTV systems, and can be easily purchased in all sort of shapes and sizes from manufacturers on Alibaba.  The low cost (less than $20 a piece), and ease of interfacing with the manufacturing makes acquiring optics easy.

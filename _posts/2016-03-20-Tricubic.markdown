...
I wanted a straightforward, easy-to-install, and fast way of smoothly interpolating values on a three dimensional <a href="https://en.wikipedia.org/wiki/Regular_grid">regular grid</a> in Python and set out to do it myself. While there are implementations of <a href="https://en.wikipedia.org/wiki/Tricubic_interpolation"> tricubic interpolation</a> in a number of languages (including Python), there wasn't one that matched all of these requirements.  I wrote one in C which could be accessed by numpy using f2py. Building off the original work by <i>Lekien and Marsden </i> I added an additional finite difference matrix and an optimization through coefficient reuse.  I implemented this within a code package called <a href="http://eqtools.readthedocs.io/en/stable/"> Eqtools </a>. Useful outside of this specific application, it can compete or beat the <a href="http://www.netlib.org/dierckx/"> equivalent bicubic interpolating splines</a> in speed (in some cases).

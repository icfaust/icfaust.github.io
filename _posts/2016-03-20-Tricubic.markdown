...
I wanted a straightforward, fast, and easy to implement way of calculating interpolated values on a three dimensional regular grid in python. While there are implementations of <a href="https://en.wikipedia.org/wiki/Tricubic_interpolation"> tricubic interpolation</a> in a number of languages (including python), there wasn't one that matched my desires.  So I wrote one myself in C which could be accessed by python/numpy using f2py. Building off the original work by <i>Lekien and Marsden </i> I added an additional finite difference matrix and an optimization through coefficient reuse.  I implemented this within a code package called <a href="http://eqtools.readthedocs.io/en/stable/"> Eqtools </a> written by some coworkers and I. Useful outside of this specific application, it compete or beat the equivalent bicubic interpolating splines in speed.

...
Other more complicated (and possibly more accurate) methods exist for generating the edge derivatives yielding more `natural' and accurate solutions.  However, I decided that the cost in implementing those solutions would be not worth the effort, as well coniditions edges truly require better choice by the users.  If the derivatives near the edge are not well-coniditioned, then the problem may be ill-posed.  This simple,mentation was only meant to yield plausably accurate solutions in the cases where the entire grid is naively interpolated upon.  
